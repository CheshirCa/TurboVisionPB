...
В Classic theme тень окна сохраняет символы заднего окна и показывает их dark gray на
чёрном; тень raised-кнопки также использует dark gray. Окно использует классические две полные клетки справа и одну строку
